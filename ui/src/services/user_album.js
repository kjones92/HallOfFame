...
const getUserAlbums = async (favourite, owned) => {
  ...
  try {
    const response = await FetchInstance(
      `users/albums?favourite=${favouriteValue}&owned=${ownedValue}`
    );
...
const getUserAlbum = async (albumId) => {
  try {
    const response = await FetchInstance(`users/albums/${albumId}`);
    return await response.json();
  } catch {
...
const setUserAlbum = async (is_owned, is_favourite, albumId) => {
  return await FetchInstance(`users/albums/${albumId}`, {
    method: "PUT",
